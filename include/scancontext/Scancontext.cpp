...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( min_dist < SC_DIST_THRES )
    {
        ...
        printf("\033[1;32m[SC Loop found] Nearest distance: %.3f btn %lu and %d.\033[0m\n", min_dist, polarcontexts_.size()-1, nn_idx);
        printf("\033[1;32m[SC Loop found] yaw diff: %.3f deg.\033[0m\n", nn_align * PC_UNIT_SECTORANGLE);
        fflush(stdout);
    }
    else
    {
        // std::cout.precision(3); 
        // printf("[Not loop] Nearest distance: %.3f btn %lu and %d.\n", min_dist, polarcontexts_.size()-1, nn_idx);
        // printf("[Not loop] yaw diff: %.3f deg.\n", nn_align * PC_UNIT_SECTORANGLE);
    }
